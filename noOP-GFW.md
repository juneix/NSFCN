...
本方案直接在 NAS 或闲置设备上部署`UIforFreedom`➕`AdGuard Home`，以原生 Linux 方式实现高性能`透明代理网关`和 `DNS 服务器`，它支持原生或 Docker 一键部署，替代了臃肿复杂的 OpenWrt 旁路由，无需虚拟机，不用额外买软路由机器，是更适合小白的轻量化方案。
...
下面是我在使用的 docker-compose 配置文件，打开你 NAS 的 Docker 套件，一键抄作业即可。  
